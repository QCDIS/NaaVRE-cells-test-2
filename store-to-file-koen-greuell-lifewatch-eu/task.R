...
make_option(c("--param_filename"), action="store", default=NA, type="character", help="my description")
)
...
id <- gsub("\"", "", opt$id)
print("Retrieving param_filename")
var = opt$param_filename
print(var)
var_len = length(var)
print(paste("Variable param_filename has length", var_len))

param_filename <- gsub("\"", "", opt$param_filename)


print("Running the cell")
package_version <- as.character(packageVersion("bioRad"))
file_content <- paste("version of biorad:", package_version, sep=" ")
