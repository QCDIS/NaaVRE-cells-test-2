...
make_option(c("--locations_input"), action="store", default=NA, type="character", help="my description"), 
...
id <- gsub("\"", "", opt$id)
print("Retrieving locations_input")
var = opt$locations_input
print(var)
var_len = length(var)
print(paste("Variable locations_input has length", var_len))

print("------------------------Running var_serialization for locations_input-----------------------")
print(opt$locations_input)
locations_input = var_serialization(opt$locations_input)
print("---------------------------------------------------------------------------------")
...
if (!dir.exists(locations_input$location_path)) {
  dir.create(locations_input$location_path)
}

stopifnot(file.exists(locations_input$input_tif_path))
stopifnot(file.exists(paste0(locations_input$input_tif_path, ".aux.xml")))

input_map <-
  rast(locations_input$input_tif_path)

bee_location <- vect(
  data.frame(
    id = locations_input$id,
    lon = locations_input$lon,
    lat = locations_input$lat
  ),
  geom = c("lon", "lat"),
  crs = "EPSG:4326"
) |>
...
lookup_table <- read.csv(locations_input$nectar_pollen_lookup_path)
...
input_patches <-
  beehave_input(input_map = input_map,
               ...
               polygon_size = 200000,
               buffer_size = locations_input$buffer_size)[[1]]

input_patches_modified <- modify_input_file(input_patches, lookup_table)

write.table(
  input_patches_modified,
  paste0(locations_input$location_path, "/input.txt"),
  sep = " ",
  row.names = FALSE
)

input_file <- paste0(locations_input$location_path, "/input.txt")
# capturing outputs
