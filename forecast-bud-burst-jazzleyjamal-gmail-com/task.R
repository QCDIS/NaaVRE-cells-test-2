...
id <- gsub("\"", "", opt$id)
print("Retrieving validation_all_zScores_file")
var = opt$validation_all_zScores_file
print(var)
var_len = length(var)
print(paste("Variable validation_all_zScores_file has length", var_len))
...
forecasting_plot
# capturing outputs
print('Serialization of year')
file <- file(paste0('/tmp/year_', id, '.json'))
writeLines(toJSON(year, auto_unbox=TRUE), file)
close(file)
